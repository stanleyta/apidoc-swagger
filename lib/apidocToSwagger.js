...
var tagsRegex = /(<([^>]+)>)/ig;
//var brTagsRegex = /(<br\s*\/?>)/ig;
// Removes <p> </p> tags from text
function removeTags(text) {
	// text = text ? text.replace(brTagsRegex, "\r\n") : text;
...
function createPostPushPutOutput(verbs, definitions, pathKeys) {
	...
	var allFields = {};
	_.merge(allFields, verbs.error ? verbs.error.fields : {});
	_.merge(allFields, verbs.success ? verbs.success.fields : {});
	var responsesPathItemObject = verbs.error ? _.transform(allFields, function(result, value, key) {
		var _key = key;
		var items = _.transform(value, function(result2, value2, key2) {
			var obj = {}
			obj[value2.field] = value2.description;
			_.merge(result2, obj);
		}, {});
	...
	pathItemObject[verbs.type].responses = responsesPathItemObject;
...
function createPathParameters(verbs, pathKeys) {
	...
	if (verbs.parameter) {
		...
		for (var i = 0; i < verbs.parameter.fields.Parameter.length; i++) {
			...
			var urlHasPathParameter = verbs.url.substring(verbs.url.lastIndexOf('/')+1).charAt(0) == ':';
			var pathParameterName = urlHasPathParameter ? verbs.url.split('/').pop().trim().slice(1) : null;
			var hasQueryString = verbs.url.indexOf('?') > -1;
			var queryStringPath = hasQueryString ? verbs.url.substring(verbs.url.lastIndexOf('?')+1) : null;
			var queryStringKeys = hasQueryString ? queryStringPath.replace(/(=)(\w)*(&)/g, ','): null;
			if (hasQueryString) queryStringKeys = queryStringKeys.substring(0, queryStringKeys.indexOf('='));
			var parameterIsInQueryString = hasQueryString ? queryStringKeys.split(',').indexOf(field) > -1 : false;
			var isQuery = hasQueryString && parameterIsInQueryString;
			var isPath = urlHasPathParameter && pathParameterName === field; //either query string or path parameter

			pathItemObject.push({
				name: field,
				in: type === "file" ? "formData" : (isQuery ? "query" : "path"),
