...
function createPathParameters(verbs, pathKeys) {
	...
	if (verbs.parameter) {
		...
		for (var i = 0; i < verbs.parameter.fields.Parameter.length; i++) {
			...
			var urlHasParameter = verbs.url.substring(verbs.url.lastIndexOf('/')+1).charAt(0) == ':';
			var parameterName = urlHasParameter ? verbs.url.split('/').pop().trim().slice(1) : null;
			var isQuery = verbs.type === "get" && (!urlHasParameter || parameterName !== field);
